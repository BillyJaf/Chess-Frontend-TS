...
const SidePanel: React.FC<Header> = ({ header } : Header) => {
  ...
  const whitePiecesDefault: { [piece: string]: number; }  = { 'P': 8, 'N': 2, 'B': 2, 'R': 2, 'Q': 1, 'K': 1 }
  const whitePieces: { [piece: string]: number; }  = { 'P': 8, 'N': 2, 'B': 2, 'R': 2, 'Q': 1, 'K': 1 }
  const blackPiecesDefault: { [piece: string]: number; }  = { 'p': 8, 'n': 2, 'b': 2, 'r': 2, 'q': 1, 'k': 1 }
  const blackPieces: { [piece: string]: number; }  = { 'p': 8, 'n': 2, 'b': 2, 'r': 2, 'q': 1, 'k': 1 }

  const displayWhitePieces = (playerColour === 'White' && header === 'Player') || (playerColour === 'Black' && header === 'Bot')

  let defaultPieces = displayWhitePieces ? whitePiecesDefault : blackPiecesDefault;
