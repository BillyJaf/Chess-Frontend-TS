...
import {
  ...
  legalMovesResponseToUILegalMoves,
} from "../../api/converters";
...
export const GameVisualsProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const {
    playerColour,
  ...
  const [visualFEN, setVisualFEN] = useState<string>(
    fenStringToVisualFen(fen, playerColour),
  );
  const [visualLegalMoves, setVisualLegalMoves] =
    useState<LegalMoves>(legalMoves);
  const [visualPieceInHand, setVisualPieceInHand] =
    useState<UIPieceInHand | null>(null);
  const [visualPromotionMove, setVisualPromotionMove] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (playerColour === fenColour) {
      fetchLegalMoves(fen).then((legalMovesResponse) => {
        const uiLegalMoves =
          legalMovesResponseToUILegalMoves(legalMovesResponse);
      ...
      fetchBestMove(fen).then((bestMoveResponse) => {
        const uiCurrentGameState =
          bestMoveResponseToUICurrentGameState(bestMoveResponse);
        setCurrentGameState(uiCurrentGameState);
      });
    }
